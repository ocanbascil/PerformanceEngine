...
class GetTest(unittest.TestCase):
  ...
  def test_hour_expiration(self):
    ...
    hour_value_hour_offset_exp = time_util.hour_expiration(5,hour_offset=2,_test_datetime=self.time)
    
    self.assertEqual(default_exp,3600)
    self.assertEqual(value_exp,18000)
    self.assertEqual(hour_offset_exp,10800) 
    self.assertEqual(hour_value_hour_offset_exp,25200)    
